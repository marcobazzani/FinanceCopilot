...
- **Privacy mode masks money, not meaning.** Every absolute currency figure rendered in `lib/ui/` MUST sit inside `PrivacyText`/`PrivacyBlur` — including rich-text/`TextSpan` bodies, `SelectableText`, info dialogs, chart labels and tooltips. Percentages, ratings, ratios, counts, dates and unit values (months/years) MUST NOT be wrapped: blurring them defeats the mode, whose point is to keep the analysis readable while hiding the amounts. Wrapping a whole `Card`/dialog to cover an amount is the usual way the second half gets broken, and it regresses silently — nothing looks wrong when too much is blurred, so a fix needs a widget test asserting both halves at once.
