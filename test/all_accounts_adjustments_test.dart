...
void main() {
  ...
  // ── "Spread spending" seeded from a transaction ───────────────────────────
  //
  // When the user taps "Spread spending" on a negative tx, the EventEditScreen
  // is pre-seeded with eventDate = tx.valueDate and totalAmount = |tx.amount|.
  // This test pins the contract: an outflow/spread event created with exactly
  // those values will automatically exclude the raw bank tx from All-Accounts
  // totals (anchor match) and replace it with the synthetic "Saving for X" rows.

  test('spread spending seeded from tx: anchor auto-excluded, saving rows injected', () async {
    final txDate = DateTime(2025, 3, 26);
    const txAmount = -20400.0;

    // Simulate what EventEditScreen saves when seeded from the T-Roc tx.
    final ev = await insEvent(name: 'T-Roc', dir: EventDirection.outflow, total: txAmount.abs(), date: txDate);
    final anchorTx = await insTx(txAmount, txDate, 'GRUPPO M SRL T-ROC');
    // Scheduled spread entries generated by the service (mocked here as two).
    final entries = [
      await insEntry(ev.id, DateTime(2025, 4, 26), -10200, EventEntryKind.scheduled),
      await insEntry(ev.id, DateTime(2025, 5, 26), -10200, EventEntryKind.scheduled),
    ];

    final res = run(events: [ev], entries: {ev.id: entries}, reimbs: {}, txs: [anchorTx]);

    // The raw -20400 tx must be annotated and excluded from totals.
    expect(res.annotatedTxIds[anchorTx.id], 'Adjusted: T-Roc', reason: 'tx seeded as eventDate/totalAmount must match anchor');
    // Scheduled entries have no matching real tx → emitted as saving rows.
    expect(res.savingItems, hasLength(2));
    expect(res.savingItems.fold<double>(0, (s, e) => s + e.amount), closeTo(-20400, 0.01));
  });

  test('spread spending: mismatched date does not exclude raw tx (user edited date in form)', () async {
    final txDate = DateTime(2025, 3, 26);
    // User changed the event date in the form — anchor no longer matches.
    final ev = await insEvent(name: 'T-Roc', dir: EventDirection.outflow, total: 20400, date: DateTime(2025, 3, 27));
    final rawTx = await insTx(-20400, txDate, 'GRUPPO M SRL');

    final res = run(events: [ev], entries: {}, reimbs: {}, txs: [rawTx]);
    expect(res.annotatedTxIds, isEmpty, reason: 'date mismatch → no anchor hit → tx not excluded');
  });
}
