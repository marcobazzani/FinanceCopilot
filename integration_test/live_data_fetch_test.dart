  ...
  testWidgets('Full price pipeline: stock ETF, bond ETF, bonds, ETC', (tester) async {
    final db = await pumpApp(
      ...
      seed: (db) async {
        ...
        final a1 = await seedAsset(db, name: 'iShares MSCI World', isin: 'IE00B4L5Y983', ticker: 'SWDA', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a1, date: DateTime(2024, 1, 15), amount: 955.0, quantity: 10, price: 95.5);

        // 2. Bond ETF — iShares EUR Govt Bond 1-3yr (Milan)
        final a2 = await seedAsset(db, name: 'iShares EUR Govt 1-3', isin: 'IE00B14X4Q57', ticker: 'IBGS', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a2, date: DateTime(2024, 3, 1), amount: 1500.0, quantity: 10, price: 150.0);

        // 3. Italian Government Bond — BTP 2.8% 2028
        final a3 = await _seedBond(db, name: 'BTP 2.80% 2028', isin: 'IT0005340929', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a3, date: DateTime(2024, 2, 1), amount: 9800.0, quantity: 100, price: 98.0);

        // 4. EU Bond — European Union 3.375% 2042 (Milan)
        final a4 = await _seedBond(db, name: 'EU 3.375% 2042', isin: 'EU000A3K4DV0', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a4, date: DateTime(2024, 4, 1), amount: 9500.0, quantity: 100, price: 95.0);

        // 5. US Treasury Bond — iShares $ Treasury 1-3yr (Milan)
        final a5 = await seedAsset(
          db,
          name: 'iShares USD Treasury 1-3',
          isin: 'IE00B3VWN179',
          ticker: 'CSBGU3',
          exchange: 'MIL',
          currency: 'EUR',
        );
        await seedBuyEvent(db, assetId: a5, date: DateTime(2024, 1, 15), amount: 1080.0, quantity: 10, price: 108.0);

        // 6. ETC — WisdomTree Physical Gold (Milan)
        final a6 = await seedAsset(db, name: 'WisdomTree Physical Gold', isin: 'JE00B1VS3770', ticker: 'PHAU', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a6, date: DateTime(2024, 1, 15), amount: 1700.0, quantity: 10, price: 170.0);

        // 7. US Stock — Amazon (NYSE)
        final a7 = await seedAsset(db, name: 'Amazon', isin: 'US0231351067', ticker: 'AMZN', exchange: 'NYQ', currency: 'USD');
        await seedBuyEvent(db, assetId: a7, date: DateTime(2024, 6, 1), amount: 1800.0, quantity: 10, price: 180.0);

        // 8. Emerging Markets ETF — iShares Core MSCI EM (Milan)
        final a8 = await seedAsset(db, name: 'iShares Core MSCI EM', isin: 'IE00BKM4GZ66', ticker: 'EIMI', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a8, date: DateTime(2024, 3, 1), amount: 400.0, quantity: 10, price: 40.0);

        // 9. Commodity ETC — UBS CMCI Composite (Milan)
        final a9 = await seedAsset(db, name: 'UBS CMCI Composite', isin: 'IE00B53H0131', ticker: 'CCUSAS', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a9, date: DateTime(2024, 5, 1), amount: 1050.0, quantity: 10, price: 105.0);

        // 10. Small Cap ETF — Xtrackers MSCI EU Small Cap (Xetra)
        final a10 = await seedAsset(
          db,
          name: 'Xtrackers EU Small Cap',
          isin: 'LU0322253906',
          ticker: 'XXSC',
          exchange: 'XETRA',
          currency: 'EUR',
        );
        await seedBuyEvent(db, assetId: a10, date: DateTime(2024, 2, 15), amount: 670.0, quantity: 10, price: 67.0);

        // 11. Italian Stock — ENEL (Milan)
        final a11 = await _seedStock(db, name: 'ENEL', isin: 'IT0003128367', ticker: 'ENEL', exchange: 'MIL', currency: 'EUR');
        await seedBuyEvent(db, assetId: a11, date: DateTime(2024, 3, 1), amount: 650.0, quantity: 100, price: 6.5);

        // 12. EU Stock — SAP (Xetra)
        final a12 = await _seedStock(db, name: 'SAP SE', isin: 'DE0007164600', ticker: 'SAP', exchange: 'XETRA', currency: 'EUR');
        await seedBuyEvent(db, assetId: a12, date: DateTime(2024, 4, 1), amount: 1800.0, quantity: 10, price: 180.0);
      },
    ...
    for (final asset in assets) {
      final prices = await (db.select(db.marketPrices)..where((p) => p.assetId.equals(asset.id))).get();
    ...
    expect(assetsWithPrices, greaterThanOrEqualTo(9), reason: 'At least 9/12 assets should have prices. Results:\n${results.join("\n")}');

    // ETFs should have TER (bonds/stocks don't)
    expect(assetsWithTer, greaterThanOrEqualTo(3), reason: 'At least 3 ETFs should have TER. Results:\n${results.join("\n")}');
    ...
    if (btpAsset != null) {
      final btpPrices = await (db.select(db.marketPrices)..where((p) => p.assetId.equals(btpAsset.id))).get();
    ...
    final swdaPrices = await (db.select(
      db.marketPrices,
    ...
    if (compositions.isNotEmpty) {
      expect(compositions.length, greaterThan(5), reason: 'Should have composition entries from multiple ETFs');
    }
  });
}
