...
class _ImportScreenState extends ConsumerState<ImportScreen> {
  ...
  // is importing position-snapshot rows (PPP TOTALEP, etc.). Each unique
  // value in the type column gets tagged as exactly one of buy/sell/
  // revalue. Pension contributions get tagged Buy (they're cash inflows
  // that grow the position — semantically identical to a discounted
  // buy). See `_parseEventType` in lib/services/import_service.dart.
  ...
  bool _canProceedToConfirm() {
    ...
    // tagged as exactly one of Buy/Sell/Revalue. The Buy/Sell pair is
    // required only for ISIN-grouped trade imports; pension statements
    // typically tag contributions as Buy and TOTALEP rows as Revalue.
    if (_target == ImportTarget.assetEvent && _typeMode == 'column' && _mappings['type'] != null) {
      ...
      if (uniqueVals.isNotEmpty) {
        ...
        // At least one tag of any kind must exist.
