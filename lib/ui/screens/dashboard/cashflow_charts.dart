...
class _YearlyBarChartState extends ConsumerState<_YearlyBarChart> with _ToggleableChartMixin {
  @override
...
class _MonthlyByYearLineChart extends ConsumerStatefulWidget {
  ...
  const _MonthlyByYearLineChart({required this.data, required this.locale, required this.language, required this.field, this.histogram = false});
...
class _MonthlyByYearLineChartState extends ConsumerState<_MonthlyByYearLineChart> with _ToggleableChartMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleYearIdx = [
      for (int i = 0; i < years.length; i++)
        if (isVisible('${years[i].year}')) i,
    ];
  ...
  Widget _buildLineChart(List<int> visibleYearIdx, double maxY, bool isPrivate, NumberFormat amtFmt, String sym, List<_YearBucket> years) {
  ...
  Widget _buildHistogram(List<int> visibleYearIdx, double maxY, bool isPrivate, NumberFormat amtFmt, String sym, List<_YearBucket> years) {
    final groups = <BarChartGroupData>[];
    final visibleCount = visibleYearIdx.length;
    final barW = visibleCount <= 4
        ? 18.0
        : visibleCount <= 7
        ? 13.0
        : visibleCount <= 10
