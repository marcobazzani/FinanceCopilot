...
- Build identifier (`appCommit`) is generated by `dart run scripts/write_build_info.dart <target>` and placed into the platform's native bundle location — NOT the Dart source tree. The generator picks git short SHA on clean checkouts, timestamp otherwise. Capped at 16 chars.
  - **Android**: writes `android/app/src/main/assets/build_info.txt` BEFORE `flutter build apk` so Gradle bundles it into the APK. Runtime reads it via a `MethodChannel` in `MainActivity.kt`.
  - **macOS**: writes `build/macos/.../FinanceCopilot.app/Contents/Resources/build_info.txt` AFTER `flutter build macos`. Runtime reads via `dart:io`.
  - **Windows**: writes `build/windows/x64/runner/Release/data/build_info.txt` AFTER `flutter build windows`. Runtime reads via `dart:io`.
  - All destinations are gitignored. No file ends up in `lib/` or `assets/`.
- When needed, always build first, then kill the running app, then start the new build. Never kill before the build completes.
  ```
  source .env && dart fix --apply && flutter build macos --release --dart-define=GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID --dart-define=GOOGLE_CLIENT_SECRET=$GOOGLE_CLIENT_SECRET && dart run scripts/write_build_info.dart macos && pkill -f "FinanceCopilot" 2>/dev/null; open build/macos/Build/Products/Release/FinanceCopilot.app
  ```
- Android APK build:
  ```
  source .env && dart run scripts/write_build_info.dart android && flutter build apk --release --dart-define=GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID --dart-define=GOOGLE_CLIENT_SECRET=$GOOGLE_CLIENT_SECRET --dart-define=GOOGLE_WEB_CLIENT_ID=$GOOGLE_WEB_CLIENT_ID --dart-define=GOOGLE_ANDROID_CLIENT_ID=$GOOGLE_ANDROID_CLIENT_ID
  ```
...
- Steps (in order):
  ...
  3. Build APK: `source .env && dart run scripts/write_build_info.dart android && flutter build apk --release --dart-define=GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID --dart-define=GOOGLE_CLIENT_SECRET=$GOOGLE_CLIENT_SECRET --dart-define=GOOGLE_WEB_CLIENT_ID=$GOOGLE_WEB_CLIENT_ID --dart-define=GOOGLE_ANDROID_CLIENT_ID=$GOOGLE_ANDROID_CLIENT_ID`
